...
public enum NombreEuro {
	...
	NombreEuro(long v, String s) {
		this(v, v, s);
	}

	NombreEuro(long min, long max) {
		this.min = min;
		this.max = max;
	}

	NombreEuro(long min, long max, String label, NombreEuro before) {
		this(min, max, label);
		this.before = before;
	}

	NombreEuro(long min, long max, String label,String addMin) {
		this(min, max, label);
		this.addMin = addMin;
	}
	
	NombreEuro(long min, long max, String label) {
		this(min, max);
		this.label = label;
	}
	...
	private String getStringValue(long value) {
		...
		NombreEuro[]values = NombreEuro.values();
		if(value<20) return values[(int)value].label;
		for (int i = 0; i < values.length; i++) {
			NombreEuro nombre = values[i];
			...
			if (value < 100 && nombre.min <= value && nombre.max >= value) {
				...
				//100 et 1000
				if ((this.equals(MILLE) || this.equals(CENT))&& NombreEuro.UN.equals(nombre))
					add.append(label);
				else{
