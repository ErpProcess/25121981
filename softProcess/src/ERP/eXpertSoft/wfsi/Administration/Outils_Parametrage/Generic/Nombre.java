...
public enum Nombre {
	...
	CALCULATE(){
		...
		public String getValue(double value,String separator)throws Exception {
		      ...
		      DecimalFormat decimForm=new DecimalFormat("0.000");
			...
			if (value == 0) return ZERO.label+" "+separator;
				...
				String vstr = decimForm.format(value);
				
				System.out.println("sssssssssssssss"+vstr);
				...
				if(indexOf == -1){
					...
					double ddd=Double.parseDouble(vstr.substring(0, indexOf))  ;
					if(ddd!=0){
					...
					String floatting =vstr.substring(indexOf+1,vstr.length());
					long v = Long.parseLong(floatting);
					System.out.println("sssssssssssssss"+v);
					if(v!=0){
						sb.append(" ");
						sb.append(" ");
