...
public class configDevelopementDAO extends  GenericWeb    {
	...
	public List<configDevelopementBean> doFindListconfigDevelopement(configDevelopementBean beanSearch) throws Exception {
		  ...
		  try{
		    String requette=" select  bean   FROM    configDevelopementBean    bean    WHERE     1=1       ";
 			   ...
 			   if(beanSearch.getFk_etab_Bean()!=null &&    !StringUtils.isEmpty(beanSearch.getFk_etab_Bean().getPk_etab().getSoc_bean().getSoc_id() ) )  
	    			requette+="   AND   bean.fk_etab_Bean.pk_etab.soc_bean.soc_id= '"+beanSearch.getFk_etab_Bean().getPk_etab().getSoc_bean().getSoc_id()+"'        ";    
 			  if(beanSearch.getFk_etab_Bean()!=null &&    !StringUtils.isEmpty(beanSearch.getFk_etab_Bean().getPk_etab().getEtab_id() ) ) 
